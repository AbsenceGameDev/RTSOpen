...
void ARTSOMassSpawner::BeginPlay()
{
	// Make sure it fires if we spawn more entities using the spawner during runtime
...
void ARTSOMassSpawner::ProcessEntityTypes()
{
	...
	{
		const TSoftObjectPtr<UMassEntityConfigAsset>& EntityConfig = EntityType.EntityConfig;
		
		if (EntityConfig == nullptr) { continue; }

		const FMassEntityTemplate& EntityTemplate = EntityConfig->GetOrCreateEntityTemplate(*GetWorld());
		if (EntityTemplate.IsValid() == false) { continue; }

		const FMassArchetypeHandle& Archetype = SpawnerSystem->GetMassEntityTemplate(EntityTemplate.GetTemplateID())->GetArchetype();
		RTSSubsystem->AssociateArchetypeWithConfigAsset(Archetype, EntityConfig);
	}
}
