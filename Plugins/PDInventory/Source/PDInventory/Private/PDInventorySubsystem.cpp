...
{
	return GetDefaultDatum(NameToTagMap.FindRef(RowName));	
}

const FPDItemDefaultDatum* UPDInventorySubsystem::GetDefaultDatum(const FGameplayTag& Tag)
{
	if (TagToItemMap.Contains(Tag) == false) { return nullptr; }
	return *TagToItemMap.Find(Tag);
}
