...
#include "AI/StateTree/PDBehaviourSchema.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PDBehaviourSchema)
...
bool UPDBehaviourSchema::IsStructAllowed(const UScriptStruct* InScriptStruct) const
{
	// Allow Mass evaluators and tasks,and common conditions. and allow BP state tree tasks, evaluators and conditions to also be allowed
...
bool UPDBehaviourSchema::IsClassAllowed(const UClass* InClass) const
{
	return IsChildOfBlueprintBase(InClass);
}

bool UPDBehaviourSchema::IsExternalItemAllowed(const UStruct& InStruct) const
{
...
void UPDBehaviourSchema::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UPDBehaviourSchema::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	Super::PostEditChangeChainProperty(PropertyChangedEvent);

	const FProperty* Property = PropertyChangedEvent.Property;
	if (Property != nullptr && Property->GetOwnerClass() == UPDBehaviourSchema::StaticClass()
		&& Property->GetFName() == GET_MEMBER_NAME_CHECKED(UPDBehaviourSchema, ContextActorClass))
	{
		ContextDataDescs[0].Struct = ContextActorClass.Get();
	}
}
